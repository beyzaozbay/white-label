...
public class MainActivityy extends AppCompatActivity {
        ...
        try {
            if (response.has("on-prem")) {
                ...
                if ("burak23".equals(value)) {
                    ...
                    changeIcon(""); // Reset to default if not "burak23"
                }
            } else {
    ...
    private void changeIcon(String value) {
        ...
        // Get the current launcher activity
        ComponentName currentLauncherActivity = packageManager.getLaunchIntentForPackage(packageName).getComponent();

        // Check if the value is "burak23"
        if ("burak23".equals(value)) {
            // Update the icon and label of the current launcher activity
            packageManager.setComponentEnabledSetting(currentLauncherActivity, PackageManager.COMPONENT_ENABLED_STATE_DISABLED, PackageManager.DONT_KILL_APP);

            // Set the new icon and label
            packageManager.setComponentEnabledSetting(new ComponentName(packageName, packageName + ".MainActivityAliasA"),
                    PackageManager.COMPONENT_ENABLED_STATE_ENABLED, PackageManager.DONT_KILL_APP);

        } else {
            // Reset to default if not "burak23"
            packageManager.setComponentEnabledSetting(currentLauncherActivity, PackageManager.COMPONENT_ENABLED_STATE_ENABLED, PackageManager.DONT_KILL_APP);
